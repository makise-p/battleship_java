...
public class BsgameAIVSSelfControl{
...
class Game{
	...
	public Game(){
		...
		players[1] = new SelfControlPlayer(this);//thisはgame
			...
			for(int i = 0; i < players.length; i++){//playersの順番（交互）にアクセスする
				...
				for(int j = 0; j < players.length; j++){//プレイヤーの数
					if(players[j].isDeadAll()){
						...
						if(j == 1){
							System.out.println("ＡＩの勝利！！！");
						}else if(j == 0){
							System.out.println("君の勝利！！！");
						}
						break;
					}
				}
	...
	public void drawField() {
		...
		for(int i = 0; i < players.length; i++){
			for(int j = 0; j < players[i].ships.size(); j++){
				...
				if(j > 0){
					if(players[i].ships.get(j) == players[i].ships.get(j-1)){
						break;
					}
				}
...
class FriendPlayer extends Player{//継承したクラスと同じ扱いができる、FriendPlayerはPlayerクラスにキャストできる
    public FriendPlayer(Game _game){
        ...
        Ship Warship = new Ship("Ｗ", rndPoint(game.width, game.height), 3, this);//thisはFriendPlayerのアドレスを渡すため、Shipから渡せるようにする
...
class EnemyPlayer extends Player{
    ...
    public void receiveAttackNotification(Player player, AttackShipNotification a_notification){//ここに攻撃を入れると、相手の攻撃がない場合は一切行動しなくなる
    	//水しぶき(攻撃のあった座標)を取得できる
    	System.out.println("君の攻撃！！");
    	System.out.println("水しぶきあり");
    	System.out.println(a_notification.name + "が(" + (a_notification.point.y+1) + "," + (a_notification.point.x+1) + ")の近くにいるようだ！");
    	
    	Point attackSamePoint = new Point(a_notification.point.x, a_notification.point.y);
    	
    	for(int i = 0; i < ships.size(); i++){
			Ship ship = ships.get(i);
			Point difference = new Point(ship.point.x - a_notification.point.x, ship.point.y - a_notification.point.y);//攻撃があった地点と自艦の座標の距離
			if(-1 <= difference.x && difference.x <= 1 && -1 <= difference.y && difference.y <= 1){//攻撃された地点の周囲に自艦が生きていること！
				//攻撃できる範囲で自艦がいれば攻撃
				ship.attack(attackSamePoint);
				AttackFlag = true;
				break;
			}
    	}
    	...
    	if(!AttackFlag){
    		...
    		if(choose > 1){
				...
				Point attackPoint = pointList.get((int)(Math.random() * pointList.size()));
				ship.attack(attackPoint);
				AttackFlag = true;//これ以降攻撃することがないから必要ないと思うけど一応つけとく
    		}else{
    			int targetIndex = (int)(Math.random() * ships.size());//艦の残存数 //動かす艦をランダムに選ぶ
    			Ship ship = ships.get(targetIndex);//shipsの配列からランダムに選ばれたのを取りだす
    			//移動
...
class SelfControlPlayer extends Player{
    ...
    public void think(){
        ...
        //残存艦の名前を表示したい
        //System.out.println(); //ちょっと無理っぽい
        System.out.println("1.Ｗ(戦艦)");
        System.out.println("2.Ｃ(巡洋艦)");
        System.out.println("3.Ｓ(潜水艦)");
        
        Scanner Sship = new Scanner(System.in);//閉じると次のSystem.inが使えなくなる
        int targetship = Sship.nextInt();
        if(targetship == 1){
        		/*
        		System.out.println("Ｗ(戦艦)はすでに轟沈しています。");//実装できそうにない、轟沈してるかどうかの判定
        		*/
        	target = 0;
        }else if(targetship == 2){
        	target = 1;
        }else if(targetship == 3){
        	target = 2;
        }
        Ship ship = ships.get(target);
        
        System.out.println("攻撃したいときは１");
        System.out.println("移動したいときは２を入力してください");
        Scanner AttackorMove = new Scanner(System.in);
        int choice = AttackorMove.nextInt();
        if(choice == 1){
        	//ランダムになってる
        	List<Point>pointList = ship.getAttackRange(game.width, game.height);//攻撃可能範囲をもらう
        	Point attackPoint = pointList.get((int)(Math.random() * pointList.size()));//攻撃地点を選択
        	ship.attack(attackPoint);//攻撃地点が有効か判断
        }else if(choice == 2){
        	//ランダムになってる
        	List<Point> pointList = ship.getMoveRange(game.width, game.height, ships);//いけないところは省かれた移動可能範囲
			Point movePoint = pointList.get((int)(Math.random() * pointList.size()));//行動可能範囲からランダムに移動
